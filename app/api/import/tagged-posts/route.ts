...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: `${statements.length}件のタグ付け投稿をインポートしました${skippedRows > 0 ? `（${skippedRows}行スキップ）` : ''}`, rowCount: statements.length });
  } catch (err) {
    console.error('Tagged posts import error:', err);
